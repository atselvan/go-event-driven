...
func celsiusToFahrenheitHandler(msg *message.Message) ([]*message.Message, error) {
	temp, err := celsiusToFahrenheit(string(msg.Payload))
	if err != nil {
		return nil, err
	}
	newMsg := message.NewMessage(watermill.NewUUID(), []byte(temp))
