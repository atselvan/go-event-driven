...
var (
	EventProductOutOfStock  Event = "ProductOutOfStock"
	EventProductBackInStock Event = "ProductBackInStock"
)
...
func NewMessageHeader(event Event) MessageHeader {
...
func (p Publisher) PublishProductOutOfStock(productID string) error {
	event := ProductOutOfStock{
		Header:    NewMessageHeader(EventProductOutOfStock),
		ProductID: productID,
	}
...
func (p Publisher) PublishProductBackInStock(productID string, quantity int) error {
	event := ProductBackInStock{
		Header:    NewMessageHeader(EventProductBackInStock),
		ProductID: productID,
		Quantity:  quantity,
	}
